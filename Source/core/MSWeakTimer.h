...
/**
 `MSWeakTimer` behaves similar to an `Timer` but doesn't retain the target.
 This timer is implemented using GCD, so you can schedule and unschedule it on arbitrary queues (unlike regular Timers!)
 */
 ...
 * Since it doesn't retain the delegate, unlike a regular `Timer`, your `dealloc` method will actually be called
